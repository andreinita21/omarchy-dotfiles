#!/usr/bin/env python3
"""Append or replace a marked hl.config() block in ~/.config/hypr/looknfeel.lua."""
...
def write_text_atomic(path: str, text: str) -> None:
    ...
    fd, tmp = tempfile.mkstemp(prefix=".looknfeel.", suffix=".tmp", dir=parent)
    try:
...
def main() -> int:
    if len(sys.argv) < 3:
        print("usage: install-looknfeel.py PLUGIN_ID LUA_BLOCK", file=sys.stderr)
        return 2
    ...
    path = os.path.join(config_home, "hypr", "looknfeel.lua")
