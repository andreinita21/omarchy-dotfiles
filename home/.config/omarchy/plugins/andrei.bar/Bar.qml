...
Item {
  ...
  // Pinned to JetBrainsMono Heavy, a renamed copy of the ExtraBold face (the
  // heaviest JetBrains Mono ships, weight 800, in ~/.local/share/fonts)
  // so every bar widget renders heavy. This no longer follows
  // `omarchy font set`; restore `Style.font.family` to track it again.
  property string fontFamily: "JetBrainsMono Heavy"
  ...
  // In a managed mode the bar background is ours, not the wallpaper, so the
  // wallpaper-sampled transparent foreground no longer describes what the text
  // actually sits on.
  property color barForeground: barModeManaged ? barModeForeground
  ...
  // ------------------------------------------------------------- bar modes
  //
  // Three appearance modes cycled with SUPER+CTRL+UP/DOWN, deliberately
  // independent of the active theme: "transparent" leaves Omarchy's own
  // behaviour untouched, "blur" paints a dark scrim that Hyprland blurs
  // behind, and "oled" paints pure black for an OLED panel. Persisted in
  // barModePath and pushed live over IPC by omarchy-bar-mode.
  readonly property string barModePath: stateHome + "/omarchy/bar-mode"
  property string barMode: "transparent"
  readonly property bool barModeManaged: barMode === "blur" || barMode === "oled"
  // Alpha low enough for Hyprland's blur to read through, high enough to keep
  // bar text legible over a bright wallpaper.
  readonly property color barModeBackground: barMode === "oled" ? Qt.rgba(0, 0, 0, 1) : Qt.rgba(0, 0, 0, 0.45)
  readonly property color barModeForeground: readableOnBlack(themeForeground)

  // A theme picks its bar text to suit its own bar background, which may be
  // light; on a black bar that text would be invisible, so anything too dark
  // to read there is replaced with white rather than left theme-dependent.
  function readableOnBlack(candidate) {
    var c = candidate
    if (typeof c === "string") c = Qt.color(c)
    // Rec. 709 luma.
    var luma = 0.2126 * c.r + 0.7152 * c.g + 0.0722 * c.b
    return luma >= 0.45 ? c : Qt.rgba(1, 1, 1, 1)
  }

  function setBarMode(value) {
    var next = String(value || "").trim()
    if (next !== "transparent" && next !== "blur" && next !== "oled") return
    root.barMode = next
    // "transparent" means no bar background on every theme, whatever the
    // theme's bar colour or shell.json's bar.transparent say. It still goes
    // through the requested-transparency path so bar text is picked to read
    // against the wallpaper behind it.
    var config = Util.isPlainObject(barConfig) ? barConfig : fallbackBarConfig
    root.setRequestedTransparency(next === "transparent" || config.transparent === true)
  }
  ...
  function applyBarConfig() {
    ...
    setRequestedTransparency(config.transparent === true || barMode === "transparent")
  ...
  //
  // A probe already in flight may have read the flag before the latest flip,
  // so a request that lands while it runs is queued and re-run on exit instead
  // of being dropped (rapid presses otherwise leave the bar out of sync).
  property bool barHiddenProbeQueued: false
  function syncBarHidden() {
    if (barHiddenProbe.running) barHiddenProbeQueued = true
  ...
  Process {
    ...
    onExited: if (root.barHiddenProbeQueued) {
  ...
  FileView {
    ...
    watchChanges: true
    printErrors: false
    onFileChanged: root.syncBarHidden()
  }

  // Seeds barMode at startup and re-reads it if the file is edited by hand.
  // omarchy-bar-mode also pushes the new mode over IPC so a keypress applies
  // without waiting on the watch.
  Process {
    id: barModeProbe
    running: true
    command: ["bash", "-c", "cat \"$HOME/.local/state/omarchy/bar-mode\" 2>/dev/null || echo transparent"]
    stdout: SplitParser { onRead: function(line) { root.setBarMode(line) } }
  }
  FileView {
    path: root.stateHome + "/omarchy"
    watchChanges: true
    printErrors: false
    onFileChanged: barModeProbe.running = true
  }
  ...
  IpcHandler {
    ...
    // Set directly by ~/.local/bin/omarchy-bar-toggle, so a keypress never
    // depends on the probe or the directory watch.
    function setHidden(hidden: string): void {
      root.barHidden = hidden === "yes" || hidden === "true"
    }

    // Applied straight away so a keypress doesn't wait on the directory watch.
    function setMode(mode: string): void {
      root.setBarMode(mode)
    }
  }
  ...
  component BarPanel: PanelWindow {
    ...
    color: root.barModeManaged ? root.barModeBackground
      : root.barMode === "transparent" || root.transparent ? "transparent" : root.background
